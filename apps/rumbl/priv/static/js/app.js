...
var Video = {
  init: function init(socket, element) {
    ...
    var videoId = element.dataset.id;
  ...
  onReady: function onReady(videoId, socket) {
    ...
    msgContainer.addEventListener("click", function (e) {
      ...
      var time = e.target.dataset.seek || e.target.parentNode.dataset.seek;
      if (!time) {
        return;
      }

      _player2.default.seekTo(time);
    });
  },
  ...
  scheduleMessages: function scheduleMessages(msgContainer, annotations) {
    ...
    setTimeout(function () {
      var time = _player2.default.getCurrentTime();
      var remaining = _this3.renderAtTime(annotations, time, msgContainer);

      _this3.scheduleMessages(msgContainer, remaining);
    }, 1000);
  },
  renderAtTime: function renderAtTime(annotations, time, msgContainer) {
    var _this4 = this;
